...
export default function PortalEntregas(){
  ...
  return (
    <div className="px-app" style={{background:C.bg,minHeight:"100%",color:C.ink,fontFamily:"'Raleway','Segoe UI',system-ui,sans-serif"}}>
      ...
      {secao!=="mentoria" && secao!=="curso" && <FaixaEtapas etapa={etapaAtual} completo={etapasFeitas} expandida={secao==="inicio"} ir={irParaEtapa}/>}
    ...
    <div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:"14px"}}>
        <div style={{display:"flex",flexWrap:"wrap",gap:"8px"}}>
          ...
          {(filtro.trim()||mostrarTodos?processosFiltrados:processosFiltrados.slice(0,3)).map(p=>(
            <button key={p.id} onClick={()=>setSelId(p.id)}
              ...
              <div style={{fontSize:"12.5px",fontWeight:700,color:C.navy}}>{p.numero_sei_processo||"sem nº SEI ainda"}</div>
              {isAdmin && <div style={{fontSize:"12px",marginTop:"2px"}}>{p.mentor_nome}</div>}
              <div style={{fontSize:"10.5px",color:C.faint,marginTop:"2px"}}>{new Date(p.criado_em).toLocaleDateString("pt-BR")}</div>
        ...
        {!filtro.trim() && processosFiltrados.length>3 && <button className="px-btn-ghost" onClick={()=>setMostrarTodos(s=>!s)} style={{alignSelf:"flex-start",padding:"5px 12px",fontSize:"11.5px"}}>
          {mostrarTodos?"Mostrar menos":`Ver todos (${processosFiltrados.length})`}
        </button>}
...
function ProcessoCredenciamentoDetalhe({processo,token,isAdmin,onMudou,onFechar}){
  ...
  async function carregarDetalhe(definirEtapaInicial){
    try{
      ...
      const dados=await resp.json();
      if(!resp.ok) return;
      const p=dados.find(x=>x.id===processo.id)||null;
      setDetalhe(p);
      if(definirEtapaInicial && p){
        const chaves=ETAPAS_CRED.map(([c])=>c);
        const atual=chaves.find(c=>{
          const et=(p.etapas||[]).find(e=>e.etapa===c);
          return et?.status!=="concluida";
        });
        setEtapaAtiva(atual||chaves[chaves.length-1]);
      }
    }catch{}
  }
  useEffect(()=>{ carregarDetalhe(true); },[processo.id]); // eslint-disable-line
